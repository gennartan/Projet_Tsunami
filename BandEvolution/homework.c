...
femSolver			*femSolverCreate(int size,int band);
void 				 femSolverFree(femSolver *mySolver);
void				 femSolverInit(femSolver *mySolver);
void				 femSolverAssemble(femSolver *mySolver, double *Aloc, double *Bloc, int *map, int nLoc);
...
int 				 femTsunamiComputeBand(femTsunami *myTsunami);
void 				 femTsunamiRenumber(femTsunami *myTsunami, femRenumType renumType);
...
femTsunami *femTsunamiCreate(const char *meshFileName, double dt){
	...
	myTsunami->size = size;
	myTsunami->number = malloc(sizeof(int)*size);
	...
	femTsunamiRenumber(myTsunami, FEM_XNUM);

	convertTo2D(myTsunami);
	myTsunami->band = femTsunamiComputeBand(myTsunami);
	myTsunami->solver = femSolverCreate(9,myTsunami->band);
...
void femTsunamiMultiplyInverseMatrix(femTsunami *myProblem)
{
    ...
    for (iElem = 0; iElem < theMesh->nElem; iElem++) {
        ...
        femSolverAssemble(theSolver,Aloc,&BE[mapElem[0]],mapE,3); 
        femSolverAssemble(theSolver,Aloc,&BU[mapElem[0]],mapU,3); 
        femSolverAssemble(theSolver,Aloc,&BV[mapElem[0]],mapV,3);     
...
void femTsunamiRenumber(femTsunami *myTsunami, femRenumType renumType){
	...
	switch(renumType){
		case FEM_NO :
			for(i=0;i<myTsunami->size;++i)
				myTsunami->number[i] = i;
		break;
		case FEM_XNUM :
		case FEM_YNUM :
			inverse = malloc(sizeof(int)*myTsunami->size);
			for(i=0;i<myTsunami->size;++i){
				inverse[i] = i;
			}
			if(renumType == FEM_XNUM) theGlobal = theMesh->X;
			if(renumType == FEM_YNUM) theGlobal = theMesh->Y;
			qsort(inverse, myTsunami->size, sizeof(int), compare);
			for(i=0;i<myTsunami->size;++i){
				myTsunami->number[inverse[i]] = i;
			}
			free(inverse);
		break;
...
int femTsunamiComputeBand(femTsunami *myTsunami){
	...
	for(iElem=0;iElem<theMesh->nElem;++iElem){
		for(j=0;j<3;++j){
			map[j] = myTsunami->number[theMesh->elem[3*iElem+j]];
		}
		myMin = map[0];
		myMax = map[0];
		for(j=1;j<3;++j){
...
femSolver *femSolverCreate(int size, int band){
	femSolver *mySolver = malloc(sizeof(mySolver));
	mySolver->B = malloc(sizeof(double)*size*(band+1));
	mySolver->A = malloc(sizeof(double)*size);
	mySolver->size = size;
	mySolver->band = band;
	mySolver->A[0] = mySolver->B + size; // WTF ?
	int i;
	for(i=1;i<size;++i)
		mySolver->A[i] = mySolver->A[i-1] + band - 1;
...
void femSolverAssemble(femSolver *mySolver, double *Aloc, double *Bloc, int *map, int nLoc){
	...
	for(i=0;i<nLoc;++i){
		...
		for(j=0;j<nLoc;++j){
			int myCol = map[j];
			if(myCol >= myRow) mySolver->A[myRow][myCol] += Aloc[i*nLoc+j];
		}
		mySolver->B[myRow] += Bloc[i];
	}
}
